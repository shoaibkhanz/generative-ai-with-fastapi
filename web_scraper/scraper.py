...
def parse_inner_text(html_string: str) -> str:
    ...
    if content := soup.find("div", "md-content"):  # , id="mw-content-text"):
        return content.get_text()
    logger.warning("Could not parse the HTML content")
    return ""


# fetch is a coroutine function since it uses await
...
async def fetch_all(urls: list[str]) -> str:
    async with aiohttp.ClientSession() as session:
        ...
        if len(success_results) != len(results):
            logger.warning("some results could not be fetched")
